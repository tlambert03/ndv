    ...
    **kwargs: ArrayDisplayModelKwargs
    ...
    def __init__(
        ...
        **kwargs: Unpack[ArrayDisplayModelKwargs],
        ...
        self._slice_ctrl = ArraySliceController(display=display_model)
        ...
        self._set_model_connected(self._slice_ctrl.display)
        ...
        if self._slice_ctrl.data_wrapper is not None:
    ...
    def display_model(self) -> ArrayDisplayModel:
        ...
        return self._slice_ctrl.display
    ...
    def display_model(self, model: ArrayDisplayModel) -> None:
        ...
        self._set_model_connected(self._slice_ctrl.display, False)
        self._slice_ctrl.display = model
        self._set_model_connected(self._slice_ctrl.display)
        self._fully_synchronize_view()
    ...
    def data_wrapper(self) -> DataWrapper | None:
        ...
        return self._slice_ctrl.data_wrapper

    @property
    def data(self) -> Any:
        """Return data being displayed."""
        if self._slice_ctrl.data_wrapper is None:
            return None  # pragma: no cover
        # returning the actual data, not the wrapper
        return self._slice_ctrl.data_wrapper.data
    ...
    def _set_data_wrapper(self, data: Any | None) -> None:
        ...
        self._slice_ctrl.data_wrapper, old = _new, self._slice_ctrl.data_wrapper
        if old is not None:
    ...
    def clone(self) -> ArrayViewer:
        ...
        return ArrayViewer(
            self._slice_ctrl.data_wrapper, display_model=self.display_model
        )
    ...
    def _update_channel_dtype(
        ...
        if not (ctrl := self._lut_controllers.get(channel, None)):
            return
        if dtype is None:
            if (wrapper := self._slice_ctrl.data_wrapper) is None:
                return
            dtype = wrapper.dtype
        else:
    ...
    def _fully_synchronize_view(self) -> None:
        ...
        display_model = self._slice_ctrl.display
        self._view.set_channel_mode(display_model.channel_mode)
        if (wrapper := self._slice_ctrl.data_wrapper) is not None:
            with self._view.currentIndexChanged.blocked():
                self._view.create_sliders(wrapper.coords)
            self._view.set_visible_axes(self._slice_ctrl.normed_visible_axes)
    ...
    def _on_model_current_index_changed(self) -> None:
        value = self._slice_ctrl.display.current_index
    ...
    def _on_view_current_index_changed(self) -> None:
        ...
        self._slice_ctrl.display.current_index.update(self._view.current_index())
    ...
    def _on_view_ndims_requested(self, ndims: int) -> None:
        ...
        if len(current_axes) > 2:
            ...
            if wrapper := self._slice_ctrl.data_wrapper:
            ...
            if z_ax is None or z_ax in current_axes:
                ...
                z_ax = next(
                    ax for ax in sld if ax not in self._slice_ctrl.normed_visible_axes
                )
    ...
    def _on_view_channel_mode_changed(self, mode: ChannelMode) -> None:
        self._slice_ctrl.display.channel_mode = mode
    ...
    def _update_visible_sliders(self) -> None:
        ...
        if self._slice_ctrl.data_wrapper is None:
            return
        hidden_indices: set[int] = set(self._slice_ctrl.normed_visible_axes)
        if self._slice_ctrl.display.channel_mode.is_multichannel():
            if (ch := self._slice_ctrl.normed_channel_axis) is not None:
                hidden_indices.add(ch)

        # hide singleton axes
        for ax, coord in self._slice_ctrl.normed_data_coords.items():
            if len(coord) < 2:
    ...
    def _request_data(self) -> None:
        ...
        plan = SlicePlanner.create_plan(
            display_model=self._slice_ctrl.display,
            data_wrapper=self._slice_ctrl.data_wrapper,
    ...
    def _get_values_at_world_point(self, x: int, y: int) -> dict[ChannelKey, float]:
        ...
        if (
            x < 0 or y < 0
        ) or self._slice_ctrl.display.n_visible_axes != 2:  # pragma: no cover
            return {}
