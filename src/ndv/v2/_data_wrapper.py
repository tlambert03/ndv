...
class DataWrapper(Generic[ArrayT]):
    ...
    def axis_index(self, axis: Hashable) -> int:
        """Return the integer index of the given axis name."""
        ndim = len(self.sizes())
        ...
        for i, name in enumerate(self.sizes()):
            if name == axis:
                return i
        avail: set = set(self.sizes()).union(range(ndim))
        raise KeyError(f"Unrecognized axis: {axis!r}. Available axes: {avail}")
