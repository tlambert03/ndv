...
class ChannelController:
    ...
    def __init__(self, key: LutKey, view: PLutView, model: LUTModel) -> None:
        ...
        self._hist = HistogramController(self.lut_model)
        self._hist.show()
    ...
    def _on_model_gamma_changed(self, gamma: float) -> None:
        """The gamma in the model has changed."""
        for handle in self.handles:
            handle.gamma = gamma
    ...
    def update_texture_data(self, data: np.ndarray) -> None:
        ...
        if not (handles := self.handles):
            return
        handles[0].data = data
        self._hist.stats.set_data(data)
        self._hist._hist.set_range(None)
    ...
    def add_handle(self, handle: PImageHandle) -> None:
        ...
        dt = np.dtype(handle.data.dtype)
        self._hist.stats.set_data(handle.data)
        self._hist._hist.set_domain((np.iinfo(dt).min, np.iinfo(dt).max))
        self._hist._hist.set_range(None)
